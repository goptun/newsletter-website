...
- **Reference format**: the two example PDFs (a reference newsletter) show the exact structure to mirror - subject line as 2-3 short headline fragments separated by `/`, a `Curiosidade para o dia <date>: ... E após as notícias de hoje: <ad>` opener, then one paragraph per story (`Bold-ish lead sentence: body. As informações são do site <X>.`), then a sponsored block and footer. Per the proposal, the ad teaser inside the Curiosidade paragraph and the sponsored block are dropped entirely, and each story paragraph loses its closing editorial remark but keeps the "As informações são do site X" attribution.
